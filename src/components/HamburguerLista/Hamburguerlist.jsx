...
function HamburguerList() {
  ...
  const adicionarItem = (hamburguerIndex) => {
    const hamburguer = {
      [hamburguerIndex]:
        Number(hamburguerSelecionado[hamburguerIndex] || 0) + 1,
    };
  ...
  return (
    <div className="HamburguerList">
      {hamburguer.map((hamburguer, index) => (
        <HamburguerListaItem
          key={`HamburgerList-${index}`}
      ...
      {hamburguermodal && (
        <HamburguerDetalhesModal
          hamburguer={hamburguermodal}
          closeModal={() => sethamburguermodal(false)}
        />
      )}
      …
    </div>
